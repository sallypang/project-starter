...
public class Test4Parse extends Test3Parse {
	...
	protected void reject(String input) throws Exception {
		...
		try {
			AST result = Parser.parse(input);
			verifyAST(result, false);
		}
		catch(parser.jcc.ParseException e) {
			transcript.println("Expected parsing failure");
		}
	}
	...
	private void verifyAST(AST result, boolean equals) throws Exception {
		...
		String unparsedAgain = Parser.unparse(parsedAgain);
		if(equals)
			Assert.assertEquals(pretty, unparsedAgain);
		else
			Assert.assertNotEquals(pretty, unparsedAgain);
	}
